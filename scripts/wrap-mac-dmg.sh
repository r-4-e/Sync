#!/usr/bin/env bash
# wrap-mac-dmg.sh — wrap a Conveyor-built Sync.app into a polished .dmg.
#
...
#    extracted `.app` in a DMG with the Sync background, drag-to-
...
STAGING="$(mktemp -d -t sync-dmg-staging)"
trap 'rm -rf "$STAGING"' EXIT
cp -R "$INPUT_APP" "$STAGING/"

mkdir -p "$(dirname "$OUT_DMG")"
TMP_DMG="$(mktemp -u "$(dirname "$OUT_DMG")/.sync-final-XXXX").dmg"
RW_DMG="$(mktemp -u "$(dirname "$OUT_DMG")/.sync-rw-XXXX").dmg"
...
# Every arch mounts a volume named "Sync". The previous arch's volume may not be
# fully released yet, so a plain `hdiutil attach` on the same name fails with EBUSY (16)
# or silently remounts as "/Volumes/Sync 1". Detach any stale volume first, attach
# with a retry, and use the REAL mount point hdiutil reports instead of a hardcoded path.
detach_stale_volumes() {
  for v in /Volumes/Sync /Volumes/Sync\ *; do
