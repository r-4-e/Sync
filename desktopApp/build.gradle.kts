...
// desktopApp — JVM application module for Sync Desktop.
//
...
// Append Sync-specific keys to Conveyor's generated config file and
...
tasks.named<hydraulic.conveyor.gradle.WriteConveyorConfigTask>("writeConveyorConfig") {
    ...
    doLast {
        destination.get().asFile.appendText(
            """
            |app.fsname = sync
            |app.display-name = Sync
            |app.rdns-name = com.flyn.sync
            |
...
compose.desktop {
    application {
        mainClass = "com.flyn.sync.MainKt"
                ...
                val macExtraPlistKeys =
                    ...
                    <array>
                        <dict>
                            ...
                            <string>com.flyn.sync.deeplink</string>
...
tasks.register("packageConveyorAppImage") {
    ...
    doLast {
        ...
        val iconDst = appDir.resolve("sync.png")
        if (!iconDst.exists() && iconSrc.exists()) {
            FileUtils.copyFile(iconSrc, iconDst)
        }

        val versionName = libs.versions.version.name.get()
        val desktopFile = appDir.resolve("sync.desktop")
        desktopFile.writeText(
            """[Desktop Entry]
            |Type=Application
            |Version=1.0
            |Name=Sync
            |Comment=Sync v$versionName - FOSS YouTube Music Client
            |Exec=bin/sync %u
            |Icon=sync
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=Sync
            |MimeType=x-scheme-handler/sync;
            |
            """.trimMargin(),
        )
        ...
        appRun.writeText(
            ...
            |if [ ! -f "${'$'}ICON_DIR/sync.png" ] || [ "${'$'}HERE/sync.png" -nt "${'$'}ICON_DIR/sync.png" ]; then
            |    mkdir -p "${'$'}ICON_DIR"
            |    cp "${'$'}HERE/sync.png" "${'$'}ICON_DIR/sync.png"
            ...
            |sed "s|Exec=bin/sync|Exec=${'$'}APPIMAGE_PATH|" "${'$'}HERE/sync.desktop" > "${'$'}DESKTOP_DIR/com-Flyn-sync-MainKt.desktop"
        ...
        // Conveyor's launcher lives at output/bin/sync (lowercase).
        val appExecutable = appDir.resolve("bin/sync")
...
tasks.register("buildLinuxAppImage") {
    group = "distribution"
    description = "Full Sync Desktop Linux AppImage build pipeline (vlcSetup → conveyor → AppImage)."
...
tasks.register("buildMacZipAmd64") {
    group = "distribution"
    description = "Full Sync Desktop macOS Intel .zip pipeline (vlcSetup → conveyor)."
    dependsOn(conveyorMakeMacZipAmd64)
}

tasks.register("buildMacZipAarch64") {
    group = "distribution"
    description = "Full Sync Desktop macOS Apple Silicon .zip pipeline (vlcSetup → conveyor)."
...
tasks.register("buildWindowsMsix") {
    group = "distribution"
    description = "Full Sync Desktop Windows .msix pipeline (vlcSetup → conveyor)."
