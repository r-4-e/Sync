...
/**
 * Registers the "sync://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\sync
 *     (Default) = "URL:Sync Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering sync:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Sync Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Sync.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Sync.exe
        // So we go: java.home → parent (runtime) → parent (app) → Sync.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Sync.exe")
            if (exeFile.exists()) {
